    ...
    .addEventListener("click", function (event) {
      ...
      setTimeout(function () {
        window.location.href =
          "https://how-to-leave-town.webflow.io/how-to-leave-town-p4";
      }, 4500);
    });
  ...
  ScrollTrigger.create({
    ...
    onUpdate: (self) => {
      ...
      if (progress <= 0.2) {
        player1.setVolume(Math.min(progress / 0.2, 1)); // Ensures the volume is capped at 1
      } else if (progress < 0.5) {
        player1.setVolume(1); // Keep volume at 100% from 20% to 80%
      } else {
        player1.setVolume(Math.max((1 - progress) / 0.5, 0)); // Ensures volume does not go below 0
  ...
  ScrollTrigger.create({
    ...
    onUpdate: (self) => {
      ...
      // Set volume based on progress
      if (progress <= 0.2) {
        player2.setVolume(Math.min(progress / 0.2, 1));
      } else if (progress < 0.5) {
        player2.setVolume(1); // Keep volume at 100% from 20% to 80%
      } else {
        player2.setVolume(Math.max((1 - progress) / 0.5, 0)); // Ensures volume does not go below 0
  ...
  ScrollTrigger.create({
    ...
    onUpdate: (self) => {
      ...
      // Set volume based on progress
      if (progress <= 0.2) {
        player3.setVolume(Math.min(progress / 0.2, 1));
      } else if (progress < 0.5) {
        player3.setVolume(1); // Keep volume at 100% from 20% to 80%
      } else {
        player3.setVolume(Math.max((1 - progress) / 0.5, 0)); // Ensures volume does not go below 0
  ...
  ScrollTrigger.create({
    ...
    onUpdate: (self) => {
      ...
      // Set volume based on progress
      if (progress <= 0.2) {
        player4.setVolume(Math.min(progress / 0.2, 1));
      } else if (progress < 0.5) {
        player4.setVolume(1); // Keep volume at 100% from 20% to 80%
      } else {
        player4.setVolume(Math.max((1 - progress) / 0.5, 0)); // Ensures volume does not go below 0
      }
    },
    onEnter: () => {
  ...
  ScrollTrigger.create({
    ...
    onUpdate: (self) => {
      ...
      // Set volume based on progress
      if (progress <= 0.2) {
        player5.setVolume(Math.min(progress / 0.2, 1));
      } else if (progress < 0.5) {
        player5.setVolume(1); // Keep volume at 100% from 20% to 80%
      } else {
        player5.setVolume(Math.max((1 - progress) / 0.5, 0)); // Ensures volume does not go below 0
      }
    },
    onEnter: () => {
  ...
  ScrollTrigger.create({
    ...
    onUpdate: (self) => {
      ...
      if (progress <= 0.05) {
        audio12.volume = progress / 0.05;
      } else if (progress > 0.05 && progress < 0.85) {
        audio12.volume = 1;
      } else if (progress >= 0.85) {
        audio12.volume = (1 - progress) / 0.1;
      }
    },
    onEnter: () => {
      audio12.play();
